...
export const GooglePlacesAutocomplete = forwardRef((props, ref) => {
  ...
  const buildRowsFromResults = useCallback(
    (results, text) => {
      ...
      // Ensure props.predefinedPlaces is always an array before calling .filter()
      const predefinedPlaces = props.predefinedPlaces || [];
  ...
  const _getFlatList = () => {
    ...
    // Check if the values are defined before accessing `.length`
    const stateTextLength = stateText ? stateText.length : 0;
    const predefinedPlacesLength = Array.isArray(props.predefinedPlaces)
      ? props.predefinedPlaces.length
      : 0;
  
    if (
      supportedPlatform() &&
      (stateTextLength !== 0 || predefinedPlacesLength > 0 || props.currentLocation === true) &&
        ...
        <FlatList
          ...
          ListEmptyComponent={
            listLoaderDisplayed
              ...
              : stateTextLength > props.minLength && props.listEmptyComponent
          }
          ListHeaderComponent={
            props.renderHeaderComponent && props.renderHeaderComponent(stateText)
          }
  ...
  let {
    onFocus = _onFocus, // Default to _onFocus if onFocus is not provided
    onBlur = _onBlur,   // Default to _onBlur if onBlur is not provided
    onChangeText,
    clearButtonMode,
    InputComp,
    ...userProps
  } = props.textInputProps || {}; // Default to an empty object if props.textInputProps is undefined
      ...
      {!props.textInputHide && (
        ...
        >
          ...
          <TextInputComp
            ...
            onFocus={onFocus ? (e) => { _onFocus(); onFocus(e); } : _onFocus}
            onBlur={onBlur ? (e) => { _onBlur(e); onBlur(e); } : _onBlur}
...
// Assign the displayName before export
GooglePlacesAutocomplete.displayName = 'GooglePlacesAutocomplete';

// Directly export the component
